...
function convertToRomanNumerals(num) {
  ...
  if (secondDigit) {
    ...
    if (secondDigit >= 5) {
      res += 'V';
      for (let j = 1; j <= secondDigit - 5; j += 1) {
        res += 'I';
      }
    } else {
      for (let j = 1; j <= secondDigit; j += 1) {
        res += 'I';
      }
    }
  }
...
function sortByAsc(arr) {
  ...
  function sort(left, right) {
    ...
    for (let i = left; i < right; i += 1) {
      if (result[i] < pivot) {
        [result[i], result[checkpoint]] = [result[checkpoint], result[i]];
        checkpoint += 1;
      }
    }

    [result[checkpoint], result[right]] = [result[right], result[checkpoint]];

    sort(left, checkpoint - 1);
    sort(checkpoint + 1, right);
  }
